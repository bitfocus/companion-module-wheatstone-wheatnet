	...
	self.setFeedbackDefinitions({
		...
		umix_input_on: {
			...
			subscribe: async (feedback) => {
				if (self.config.debugLogging) {
					self.log('debug', `feedback umix_input_on subscribe: ${JSON.stringify(feedback.options)}`)
				}
			...
			callback: (feedback) => {
				...
				if (self.config.debugLogging) {
					self.log(
						'debug',
						`feedback umix_input_on callback: key=${key}, state=${JSON.stringify(self.state.umix[key])}`,
					)
				}
		...
		umix_input_fdr_above: {
			...
			subscribe: (feedback) => {
				...
				if (self.config.debugLogging) {
					self.log('debug', `feedback umix_input_fdr_above subscribe: ${JSON.stringify(feedback.options)}`)
				}
		...
		umix_output_on: {
			...
			subscribe: (feedback) => {
				if (self.config.debugLogging) {
					self.log('debug', `feedback umix_output_on subscribe: ${JSON.stringify(feedback.options)}`)
				}
				const mixer = Number(feedback.options.mixer)
				const bus = String(feedback.options.bus)
			...
			callback: (feedback) => {
				...
				if (self.config.debugLogging) {
					self.log(
						'debug',
						`feedback umix_output_on callback: key=${key}, state=${JSON.stringify(self.state.umix[key])}`,
					)
				}
				// handle undefined state
				if (state === undefined) {
